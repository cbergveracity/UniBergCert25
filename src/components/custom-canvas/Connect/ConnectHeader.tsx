...
export const ConnectHeaderLevelOne: FC<ConnectFirstLevelProps> = ({ title, link, component, context, slots }) => (
    <li className="relative h-full group">
        <Link href="/" className="flex font-medium items-center h-full px-4 text-gray-800 hover:text-blue-600 transition-colors">{title}</Link>
        <div className="fixed left-0 w-full bg-white shadow-md py-6 z-50 transition-all duration-300 ease-in-out opacity-0 invisible group-hover:opacity-100 group-hover:visible border-t-2" style={{ top: "64px" }} >
            <div className="container mx-auto px-4">
...
export const ConnectHeaderLevelTwo: FC<ConnectSecondLevelProps> = ({ title, link, component, context, slots }) => (
    <div>
        <Link href="/products/category-1" className="block px-4 font-medium text-gray-800 hover:text-blue-600 mb-2 transition-colors">
            {title}
        </Link>
